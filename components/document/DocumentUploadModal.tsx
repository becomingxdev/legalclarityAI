...
} from "@/lib/sample-data/sampleContracts";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm">
      <div className="relative w-full max-w-2xl rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-slate-800 dark:bg-slate-900 sm:p-8">
        ...
        <div className="mt-6 space-y-4">
          ...
          {/* Quick Demo Contract Presets */}
          <div className="rounded-xl border border-indigo-100 bg-indigo-50/40 p-3.5 dark:border-indigo-950 dark:bg-indigo-950/20">
            <span className="text-[11px] font-bold text-indigo-900 dark:text-indigo-300 block mb-2">
              ⚡ Quick Demo Contract Presets (1-Click Load)
            </span>
            <div className="flex flex-wrap gap-2">
              <button
                type="button"
                onClick={() => {
                  setTitle(SAMPLE_EMPLOYMENT_AGREEMENT.title || "Executive Employment Agreement");
                  setTextInput(SAMPLE_EMPLOYMENT_AGREEMENT.rawText || "");
                  setFile(null);
                }}
                className="rounded-lg border border-indigo-200 bg-white px-2.5 py-1 text-[11px] font-semibold text-indigo-700 shadow-xs hover:bg-indigo-50 dark:border-indigo-800 dark:bg-slate-800 dark:text-indigo-300"
              >
              ...
              <button
                type="button"
                onClick={() => {
                  setTitle(SAMPLE_SAAS_CONTRACT.title || "CloudScale Enterprise SaaS Agreement (2025)");
                  setTextInput(SAMPLE_SAAS_CONTRACT.rawText || "");
                  setFile(null);
                }}
                className="rounded-lg border border-indigo-200 bg-white px-2.5 py-1 text-[11px] font-semibold text-indigo-700 shadow-xs hover:bg-indigo-50 dark:border-indigo-800 dark:bg-slate-800 dark:text-indigo-300"
              >
              ...
              <button
                type="button"
                onClick={() => {
                  setTitle(SAMPLE_REVISED_SAAS_CONTRACT.title || "CloudScale Enterprise SaaS Agreement (Vendor Redline v2)");
                  setTextInput(SAMPLE_REVISED_SAAS_CONTRACT.rawText || "");
                  setFile(null);
                }}
                className="rounded-lg border border-indigo-200 bg-white px-2.5 py-1 text-[11px] font-semibold text-indigo-700 shadow-xs hover:bg-indigo-50 dark:border-indigo-800 dark:bg-slate-800 dark:text-indigo-300"
              >
                📝 SaaS Agreement v2 (Redline)
              </button>
              <button
                type="button"
                onClick={() => {
                  setTitle(SAMPLE_COMMERCIAL_LEASE.title || "Commercial Office Building Lease");
                  setTextInput(SAMPLE_COMMERCIAL_LEASE.rawText || "");
                  setFile(null);
                }}
                className="rounded-lg border border-indigo-200 bg-white px-2.5 py-1 text-[11px] font-semibold text-indigo-700 shadow-xs hover:bg-indigo-50 dark:border-indigo-800 dark:bg-slate-800 dark:text-indigo-300"
              >
                🏢 Commercial Lease (NNN)
              </button>
            </div>
          </div>
